...
void LRI_CV<Tdata>::set_orbitals(
    ...
    const ModuleBase::Element_Basis_Index::Range range_abfs
        = Exx_Abfs::Abfs_Index::construct_range(abfs);
    this->index_abfs
...
auto LRI_CV<Tdata>::cal_dVs(
    ...
    const T_func_DPcal_data<std::array<RI::Tensor<Tdata>, 3>> func_DPcal_dV
        = std::bind(&LRI_CV<Tdata>::DPcal_dV,
                    this,
                    std::placeholders::_1,
                    std::placeholders::_2,
                    std::placeholders::_3,
...
auto LRI_CV<Tdata>::cal_Cs_dCs(
    ...
    const std::map<std::string, bool>&
        flags) // "cal_dC" + "writable_Cws", "writable_dCws", "writable_Vws",
               // "writable_dVws"
    -> std::pair<
        std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>,
        std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, 3>>>> {
    ModuleBase::TITLE("LRI_CV", "cal_Cs_dCs");
    const T_func_DPcal_data<
        std::pair<RI::Tensor<Tdata>, std::array<RI::Tensor<Tdata>, 3>>>
        func_DPcal_C_dC = std::bind(&LRI_CV<Tdata>::DPcal_C_dC,
                                    this,
                                    std::placeholders::_1,
                                    std::placeholders::_2,
                                    std::placeholders::_3,
...
To11 LRI_CV<Tdata>::DPcal_o11(
    ...
    std::map<int, std::map<int, std::map<Abfs::Vector3_Order<double>, To11>>>&
        o11ws,
    ...
    if (LRI_CV_Tools::exist(o11_read)) {
        ...
        pthread_rwlock_rdlock(&rwlock_o11);
        const To11 o11_transform_read
            = RI::Global_Func::find(o11ws, it1, it0, Rm);
        pthread_rwlock_unlock(&rwlock_o11);
...
template <typename Tdata>
std::array<RI::Tensor<Tdata>, 3> LRI_CV<Tdata>::DPcal_dV(
    const int it0,
    const int it1,
    const Abfs::Vector3_Order<double>& R,
    const std::map<std::string, bool>& flags) // "writable_dVws"
{
    ...
    const auto cal_grad_overlap_matrix
        = std::bind(&Matrix_Orbs11::cal_grad_overlap_matrix<Tdata>,
                    &this->m_abfs_abfs,
                    std::placeholders::_1,
                    std::placeholders::_2,
                    std::placeholders::_3,
...
std::pair<RI::Tensor<Tdata>, std::array<RI::Tensor<Tdata>, 3>>
    LRI_CV<Tdata>::DPcal_C_dC(
        ...
        const Abfs::Vector3_Order<double>& R,
        const std::map<std::string, bool>&
            flags) // "cal_dC", "writable_Cws", "writable_dCws" +
                   // "writable_Vws", "writable_dVws"
{
    ...
    const RI::Tensor<Tdata> C_read
        = RI::Global_Func::find(this->Cws, it0, it1, R);
    pthread_rwlock_unlock(&this->rwlock_Cw);
    pthread_rwlock_rdlock(&this->rwlock_dCw);
    const std::array<RI::Tensor<Tdata>, 3> dC_read
        = RI::Global_Func::find(this->dCws, it0, it1, R);
    pthread_rwlock_unlock(&this->rwlock_dCw);
    const bool flag_finish_dC
        = (!flags.at("cal_dC")) || LRI_CV_Tools::exist(dC_read);
    ...
    if (!C_read.empty() && flag_finish_dC) {
        ...
        if ((ModuleBase::Vector3<double>(0, 0, 0) == R) && (it0 == it1)) {
            ...
            const RI::Tensor<Tdata> V
                = this->DPcal_V(it0, it0, {0, 0, 0}, {{"writable_Vws", true}});
            const RI::Tensor<Tdata> L = LRI_CV_Tools::cal_I(V);

            const RI::Tensor<Tdata> C
                = RI::Global_Func::convert<Tdata>(0.5)
                  * LRI_CV_Tools::mul1(L, A); // Attention 0.5!
            ...
            if (flag_finish_dC) {
                ...
                const RI::Shape_Vector sizes
                    = {this->index_abfs[it0].count_size,
                       this->index_lcaos[it0].count_size,
                       this->index_lcaos[it0].count_size};
                const std::array<RI::Tensor<Tdata>, 3> dC(
                    {RI::Tensor<Tdata>({sizes}),
                     RI::Tensor<Tdata>({sizes}),
                     RI::Tensor<Tdata>({sizes})});
            ...
            const std::vector<std::vector<RI::Tensor<Tdata>>> V
                = {{DPcal_V(it0, it0, {0, 0, 0}, {{"writable_Vws", true}}),
                    DPcal_V(it0, it1, R, flags)},
                   {DPcal_V(it1, it0, Rm, flags),
                    DPcal_V(it1, it1, {0, 0, 0}, {{"writable_Vws", true}})}};

            const std::vector<std::vector<RI::Tensor<Tdata>>> L
                = LRI_CV_Tools::cal_I(V);
            ...
            if (flag_finish_dC) {
                ...
                const std::vector<std::array<RI::Tensor<Tdata>, 3>> dA
                    = {this->m_abfslcaos_lcaos
                           .template cal_grad_overlap_matrix<Tdata>(
                               it0,
                               it1,
                               {0, 0, 0},
                               R,
                ...
                const std::array<RI::Tensor<Tdata>, 3> dV_01
                    = DPcal_dV(it0, it1, R, flags);
                const std::array<RI::Tensor<Tdata>, 3> dV_10
                    = LRI_CV_Tools::negative(DPcal_dV(it1, it0, Rm, flags));
                ...
                const std::vector<std::array<RI::Tensor<Tdata>, 3>> dC
                    = LRI_CV_Tools::change_order(std::move(dC_tmp));
                if (flags.at("writable_dCws")) {
                    pthread_rwlock_wrlock(&this->rwlock_dCw);
                    this->dCws[it0][it1][R] = dC[0];
                    this->dCws[it1][it0][Rm] = LRI_CV_Tools::negative(
                        LRI_CV_Tools::transpose12(dC[1]));
                    pthread_rwlock_unlock(&this->rwlock_dCw);
                }
