...
template <typename Tdata>
void LRI_CV<Tdata>::set_orbitals(const LCAO_Orbitals& orb,
                                 ...
                                 const bool& init_Vr)
{
    ...
    const double abfs_ccp_rmax = Exx_Abfs::Construct_Orbs::get_Rmax(this->abfs_ccp);
    ...
    if (init_C)
    {
        this->m_abfslcaos_lcaos.init_radial(this->abfs_ccp, this->lcaos, this->lcaos, MGT);
...
{
    return std::min(this->abfs_ccp_rcut[it0], this->lcaos_rcut[it0]) + this->lcaos_rcut[it1];
}
...
auto LRI_CV<Tdata>::cal_datas(const std::vector<TA>& list_A0,
                              ...
                              const T_func_DPcal_data<Tresult>& func_DPcal_data) -> std::map<TA, std::map<TAC, Tresult>>
{
...
template <typename Tdata>
auto LRI_CV<Tdata>::cal_rVrs(const std::vector<TA>& list_A0, const std::vector<TAC>& list_A1)
    -> std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, 3>>>
{
    ModuleBase::TITLE("LRI_CV", "cal_rVrs");

    using namespace RI::Array_Operator;
    std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, 3>>> Datas;
    for (size_t i0 = 0; i0 < list_A0.size(); ++i0)
    {
        for (size_t i1 = 0; i1 < list_A1.size(); ++i1)
        {
            ...
            const ModuleBase::Vector3<double> tau0 = GlobalC::ucell.atoms[it0].tau[ia0];
            const ModuleBase::Vector3<double> tau1 = GlobalC::ucell.atoms[it1].tau[ia1];
            const double Rcut = std::min(LRI_CV<Tdata>::cal_V_Rcut(it0, it1), LRI_CV<Tdata>::cal_V_Rcut(it1, it0));
            const Abfs::Vector3_Order<double> R
                = -tau0 + tau1 + (RI_Util::array3_to_Vector3(cell1) * GlobalC::ucell.latvec);
            const Abfs::Vector3_Order<double> Rm = -R;
            std::array<RI::Tensor<Tdata>, 3> AVr = RI::Global_Func::find(this->Vrws, it0, it1, R);
            RI::Tensor<Tdata> AV = RI::Global_Func::find(this->Vws, it0, it1, R);
            std::array<RI::Tensor<Tdata>, 3> BVr = RI::Global_Func::find(this->Vrws, it1, it0, Rm);
            RI::Tensor<Tdata> BV = RI::Global_Func::find(this->Vws, it1, it0, Rm);
            if (LRI_CV_Tools::exist(AV) && LRI_CV_Tools::exist(AVr) && LRI_CV_Tools::exist(BV)
                && LRI_CV_Tools::exist(BVr))
            {
                std::array<RI::Tensor<Tdata>, 3> A;
                ...
                Datas[list_A0[i0]][list_A1[i1]] = A - LRI_CV_Tools::transform_Rm(B);
            }
        }
    }
    return Datas;
...
template <typename Tdata>
auto LRI_CV<Tdata>::cal_Vrs(const std::vector<TA>& list_A0,
                            const std::vector<TAC>& list_A1,
...
template <typename Tdata>
auto LRI_CV<Tdata>::cal_Vs(const std::vector<TA>& list_A0,
                           const std::vector<TAC>& list_A1,
    ...
    -> std::map<TA, std::map<TAC, RI::Tensor<Tdata>>>
{
    ...
    const T_func_DPcal_data<RI::Tensor<Tdata>> func_DPcal_V = std::bind(&LRI_CV<Tdata>::DPcal_V,
                                                                        this,
                                                                        std::placeholders::_1,
                                                                        std::placeholders::_2,
                                                                        std::placeholders::_3,
    ...
    const T_func_cal_Rcut func_cal_Rcut
        = std::bind(&LRI_CV<Tdata>::cal_V_Rcut, this, std::placeholders::_1, std::placeholders::_2);
    return this->cal_datas(list_A0, list_A1, flags, func_cal_Rcut, func_DPcal_V);
}

template <typename Tdata>
auto LRI_CV<Tdata>::cal_dVs(const std::vector<TA>& list_A0,
                            const std::vector<TAC>& list_A1,
                            const std::map<std::string, bool>& flags) // + "writable_dVws"
    -> std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, 3>>>
{
    ModuleBase::TITLE("LRI_CV", "cal_dVs");
    const T_func_DPcal_data<std::array<RI::Tensor<Tdata>, 3>> func_DPcal_dV = std::bind(&LRI_CV<Tdata>::DPcal_dV,
                                                                                        this,
                                                                                        std::placeholders::_1,
                                                                                        std::placeholders::_2,
                                                                                        std::placeholders::_3,
...
template <typename Tdata>
auto LRI_CV<Tdata>::cal_Cs_dCs(const std::vector<TA>& list_A0,
                               const std::vector<TAC>& list_A1,
                 ...
                 std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, 3>>>>
{
    ...
    const T_func_DPcal_data<std::pair<RI::Tensor<Tdata>, std::array<RI::Tensor<Tdata>, 3>>> func_DPcal_C_dC
        = std::bind(&LRI_CV<Tdata>::DPcal_C_dC,
                    this,
                    std::placeholders::_1,
                    std::placeholders::_2,
                    std::placeholders::_3,
                    std::placeholders::_4);
    const T_func_cal_Rcut func_cal_Rcut
        = std::bind(&LRI_CV<Tdata>::cal_C_Rcut, this, std::placeholders::_1, std::placeholders::_2);
    std::map<TA, std::map<TAC, std::pair<RI::Tensor<Tdata>, std::array<RI::Tensor<Tdata>, 3>>>> Cs_dCs_tmp
                              ...
                              const Tdata& transform_factor)
{
    ...
    else
    {
        ...
        {
            const To11 o11 = LRI_CV_Tools::mul2(transform_factor, LRI_CV_Tools::transform_Rm(o11_transform_read));
...
std::array<RI::Tensor<Tdata>, 3> LRI_CV<Tdata>::DPcal_Vr(const int it0,
                                                         ...
                                                         const std::map<std::string, bool>& flags) // "writable_Vws"
{
    pthread_rwlock_rdlock(&this->rwlock_Vrw);
    const std::array<RI::Tensor<Tdata>, 3> Vr_read = RI::Global_Func::find(this->Vrws, it0, it1, R);
    pthread_rwlock_unlock(&this->rwlock_Vrw);

    if (LRI_CV_Tools::exist(Vr_read))
    {
        return Vr_read;
    }
    else
    {
        const std::array<RI::Tensor<Tdata>, 3> Vr
            = this->m_abfs_r_abfs.template cal_overlap_matrix<Tdata>(it0,
                                                              it1,
                                                              ModuleBase::Vector3<double>{0, 0, 0},
                                                              R,
                                                              this->index_abfs,
                                                              this->index_abfs,
                                                              Matrix_Orbs11::Matrix_Order::AB);
        if (flags.at("writable_Vrws"))
        {
            pthread_rwlock_rdlock(&this->rwlock_Vrw);
            this->Vrws[it0][it1][R] = Vr;
            pthread_rwlock_unlock(&this->rwlock_Vrw);
        }
        return Vr;
    }
                                         ...
                                         const std::map<std::string, bool>& flags) // "writable_Vws"
{
    const auto cal_overlap_matrix = std::bind(&Matrix_Orbs11::cal_overlap_matrix<Tdata>,
                                              ...
                                              std::placeholders::_7);
    return this
        ->DPcal_o11(it0, it1, R, flags.at("writable_Vws"), this->rwlock_Vw, this->Vws, cal_overlap_matrix, Tdata(1.0));
}
                                                         ...
                                                         const std::map<std::string, bool>& flags) // "writable_dVws"
{
    if (ModuleBase::Vector3<double>(0, 0, 0) == R)
    {
        ...
        const std::array<RI::Tensor<Tdata>, 3> dV
            = {RI::Tensor<Tdata>({size, size}), RI::Tensor<Tdata>({size, size}), RI::Tensor<Tdata>({size, size})};
    ...
    const auto cal_grad_overlap_matrix = std::bind(&Matrix_Orbs11::cal_grad_overlap_matrix<Tdata>,
                                                   &this->m_abfs_abfs,
                                                   std::placeholders::_1,
                                                   std::placeholders::_2,
                                                   std::placeholders::_3,
    ...
    else
    {
        if ((ModuleBase::Vector3<double>(0, 0, 0) == R) && (it0 == it1))
        {
            const RI::Tensor<Tdata> A
                = this->m_abfslcaos_lcaos.template cal_overlap_matrix<Tdata>(it0,
                                                                             it1,
                                                                             {0, 0, 0},
                                                                             {0, 0, 0},
                                                                             this->index_abfs,
            ...
            const RI::Tensor<Tdata> C
                = RI::Global_Func::convert<Tdata>(0.5) * LRI_CV_Tools::mul1(L, A); // Attention 0.5!
        ...
        else
        {
            const std::vector<RI::Tensor<Tdata>> A
                = {this->m_abfslcaos_lcaos.template cal_overlap_matrix<Tdata>(it0,
                                                                              ...
                                                                              this->index_lcaos,
                                                                              Matrix_Orbs21::Matrix_Order::A1A2B),
                   this->m_abfslcaos_lcaos.template cal_overlap_matrix<Tdata>(it1,
                                                                              it0,
                                                                              {0, 0, 0},
                                                                              Rm,
                                                                              this->index_abfs,
            ...
            else
            {
                const std::vector<std::array<RI::Tensor<Tdata>, 3>> dA = {
                    this->m_abfslcaos_lcaos.template cal_grad_overlap_matrix<Tdata>(it0,
                                                                                    it1,
                                                                                    {0, 0, 0},
                                                                                    R,
                                                                                    this->index_abfs,
                                                                                    ...
                                                                                    this->index_lcaos,
                                                                                    Matrix_Orbs21::Matrix_Order::A1A2B),
                    LRI_CV_Tools::negative(this->m_abfslcaos_lcaos.template cal_grad_overlap_matrix<Tdata>(
                        it1,
                        it0,
                        {0, 0, 0},
                        Rm,
